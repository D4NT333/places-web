...
function getPhotoCountLabel(count) {
  ...
  return `${safeCount} ${safeCount === 1 ? "Foto" : "Fotos"}`;
}
...
export default function PhotoSubmissionCard({
  ...
  const statusConfig =
    STATUS_CONFIG[status] || STATUS_CONFIG.in_review;

  const finalPhotoCount =
    photoCount ?? extraPhotosCount ?? 0;
    ...
    >
      <div style={styles.imageContainer}>
        <span style={styles.photoCountBadge}>
          {getPhotoCountLabel(finalPhotoCount)}
        </span>

        {imageUrl ? (
          <img
            src={imageUrl}
            alt={`Fotografía propuesta para ${placeName}`}
            style={styles.image}
          />
        ) : (
      ...
      <div style={styles.infoSection}>
        <div style={styles.mainInformation}>
          ...
          <p style={styles.userText}>
            <span style={styles.userLabel}>Por:</span>{" "}
